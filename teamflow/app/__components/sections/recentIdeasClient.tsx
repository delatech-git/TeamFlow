...
export function RecentIdeasClient({ initialIdeas }: Props) {
  ...
  const plannedCount = initialIdeas.filter(
  ...
  return (
    <section className="relative overflow-hidden rounded-[28px] border border-white/10 bg-[#0b1020] shadow-[0_30px_80px_rgba(0,0,0,0.35)]">
      ...
      <div className="relative grid min-h-[650px] lg:grid-cols-[320px_minmax(0,1fr)] xl:grid-cols-[370px_minmax(0,1fr)]">
        <aside className="relative flex min-h-[440px] flex-col overflow-hidden border-b border-white/10 bg-gradient-to-b from-white/[0.035] to-transparent p-6 sm:p-8 lg:min-h-full lg:border-b-0 lg:border-r lg:p-10">
          ...
          <div className="pointer-events-none absolute bottom-0 left-0 right-0 z-10 h-[68%]">
            ...
            <Image
              ...
              alt=""
              priority
              className="absolute bottom-0 left-1/2 h-auto w-[300px] max-w-none -translate-x-1/2 object-contain sm:w-[340px] lg:w-[360px]"
            />
          </div>

          <div className="pointer-events-none absolute bottom-20 left-14 z-0 h-28 w-28 rotate-12 rounded-3xl border border-white/5 bg-white/[0.015]" />
          <div className="pointer-events-none absolute bottom-40 right-8 z-0 h-20 w-20 -rotate-12 rounded-2xl border border-white/5 bg-white/[0.015]" />
        </aside>
        ...
        <div className="relative p-5 sm:p-7 lg:p-8 xl:p-10">
          <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
            ...
            <Link
              href="/discover-ideas"
              className="group inline-flex h-12 shrink-0 items-center justify-center gap-3 rounded-2xl border border-orange-400/50 bg-gradient-to-r from-orange-500/10 to-pink-500/10 px-5 text-sm font-semibold text-white transition duration-300 hover:border-pink-400 hover:from-orange-500/20 hover:to-pink-500/20"
            >
              See all ideas
              <ArrowRight
                size={16}
            ...
            {topIdeas.length > 0 ? (
              <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                {topIdeas.map((idea, index) => (
                  <IdeaCard key={idea.id} idea={idea} priority={index === 0} />
                ))}
              </div>
            ) : (
              <EmptyIdeasState />
...
function IdeaCard({
  ...
  return (
    <article className="group relative flex min-h-[470px] flex-col overflow-hidden rounded-[22px] border border-white/10 bg-[#111827]/80 transition duration-300 hover:-translate-y-1 hover:border-white/20 hover:shadow-[0_24px_60px_rgba(0,0,0,0.32)]">
      <Link
      ...
      <div className="relative aspect-[4/3] overflow-hidden bg-white/5">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#111827] via-transparent to-black/10" />
...
function StatCard({
  ...
  return (
    <div
      className={`flex min-h-[88px] items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.045] px-4 py-3 backdrop-blur-md transition hover:border-white/15 hover:bg-white/[0.065] ${styles.glow}`}
    >
      <span
...
function EmptyIdeasState() {
  return (
    <div className="flex min-h-[390px] flex-col items-center justify-center rounded-[22px] border border-dashed border-white/15 bg-white/[0.025] px-6 text-center">
